...
export default function AdminAccountsPage() {
  ...
  const viewActivity = async (account) => {
    ...
    try {
        // NOTE: This assumes /api/admin/routes/ supports ?courier=ID filtering.
        // If not, it might return all routes (which we filter manually below as a fallback)
        const res = await api.get(`/api/admin/routes/?courier=${account.id}`);
        const routes = res.data.results || res.data || [];
        
        // Manual fallback filter just in case backend ignores param
        const filtered = routes.filter(r => r.courier === account.id || r.courier?.id === account.id);
        
        // Sort: Active first, then planned by date
